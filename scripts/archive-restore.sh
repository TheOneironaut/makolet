...
if [[ $# -ne 2 ]]; then
  echo "usage: scripts/archive-restore.sh BACKUP_DIRECTORY CONFIRM_BUCKET_NAME" >&2
  exit 2
fi
...
archive_container_name="$(new_archive_container_name restore)"
...
bucket="$(run_with_bounded_watchdog \
  "$archive_watchdog_timeout_seconds" \
  "${compose[@]}" --profile operations config --format json | python -c \
  'import json,sys; print(json.load(sys.stdin)["services"]["archive-tool"]["environment"]["MAKOLET_S3_BUCKET"])')"
if [[ "$2" != "$bucket" ]]; then
  echo "confirmation does not exactly match the configured archive bucket: $bucket" >&2
  exit 1
fi
...
trap finish_archive_restore EXIT
run_with_bounded_watchdog \
  ...
  "${backup_limit_arguments[@]}" \
  archive-tool restore /backup --confirm-bucket "$bucket"
